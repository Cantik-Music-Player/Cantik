...
describe "Player Component", ->
  ...
  beforeEach ->
    ...
    pluginManager = {'plugins': {'playlist': {'on': sinon.spy()}}}
    Player.prototype.show = sinon.spy()
    @player = new Player(pluginManager)
    @player.on = sinon.spy()
    @pc = new PlayerComponent({player: @player})
  ...
  it "Initialized", ->
    assert.deepEqual(@pc.state, {
      ...
      'duration': null,
      'mute': false,
      'repeat': @player.pluginManager.plugins.playlist.repeat,
      'random': @player.pluginManager.plugins.playlist.random
    })
  ...
  it "Update Random", ->
    @player.pluginManager.plugins.playlist.random = 'test'
  ...
  it "Update Repeat", ->
    @player.pluginManager.plugins.playlist.repeat = 'one'
  ...
  it "Set Random", ->
    @player.pluginManager.plugins.playlist.setRandom = sinon.spy()

    @pc.setRandom 'test'

    assert(@player.pluginManager.plugins.playlist.setRandom.calledWith('test'))

  it "Switch Repeat State", ->
    @player.pluginManager.plugins.playlist.switchRepeatState = sinon.spy()

    do @pc.switchRepeatState

    assert(@player.pluginManager.plugins.playlist.switchRepeatState.called)
  ...
  it "Render", ->
    @showPlayer(@player, document.getElementsByTagName("body")[0])

    # Clean data-react-id
    html = document.getElementsByTagName("body")[0].innerHTML.replace(/ data-reactroot=""/g, '')

    assert.equal(html,
    '<div class="panel panel-default" id="player"><audio controls=""></audio><p class="track-artist"><span class="title"></span><!-- react-text: 5 -->-<!-- /react-text --><span class="artist"></span></p><div class="panel-body"><div class="left-button"><button><i class="material-icons previous">skip_previous</i></button><button><i class="material-icons play">play_arrow</i></button><button><i class="material-icons next">skip_next</i></button></div><span class="elapsed-time"></span><div class="progress"><div class="slider shor progressbar noUi-target noUi-ltr noUi-horizontal noUi-connect"><div class="noUi-base"><div class="noUi-origin noUi-background" style="left: 0%;"><div class="noUi-handle noUi-handle-lower"></div></div></div></div></div><span class="total-time"></span><div class="volume-container"><button class="volume-button"><i class="material-icons volume-icon">volume_up</i></button><div class="slider shor volume noUi-target noUi-ltr noUi-horizontal noUi-connect"><div class="noUi-base"><div class="noUi-origin noUi-background" style="left: 50%;"><div class="noUi-handle noUi-handle-lower"></div></div></div></div></div><div class="right-button"><button class="random"><i class="material-icons">shuffle</i></button></div></div></div>')
